...
# Logging للمساعدة في تتبع الأخطاء
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# إعداد Flask مع المسارات الصحيحة لـ Render
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
app = Flask(__name__,
    template_folder=os.path.join(BASE_DIR, 'templates'),
    static_folder=os.path.join(BASE_DIR, 'static')
)
app.config['UPLOAD_FOLDER'] = os.path.join(BASE_DIR, 'static', 'uploads', 'items')
...
# تهيئة قاعدة البيانات قبل كل طلب
@app.before_request
def init_database():
    try:
        init_db()
